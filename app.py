...
/* Only colour text nodes, don't touch structural elements */
...
/* ── Hide only Streamlit footer & deploy button ── */
...
.meetmind-header .logo-ring {
    ...
    font-size: 22px; box-shadow: 0 0 24px rgba(79,142,247,0.35);
}
.meetmind-header h1 {
...
.meetmind-header span {
    color: var(--muted); font-size: 0.82rem; margin-top: 3px; display: block;
    font-family: 'Space Grotesk', sans-serif;
}
...
.item-dot { width: 7px; height: 7px; border-radius: 50%; margin-top: 7px; flex-shrink: 0; }
.item-dot.blue   { background: var(--accent); }
.item-dot.purple { background: var(--accent2); }
.item-dot.orange { background: var(--warn); }
.item-text { font-size: 0.88rem; color: #B8C3D9; line-height: 1.6; font-family: 'Space Grotesk', sans-serif; }
...
.stat-card {
    background: var(--card-bg); border: 1px solid var(--border);
    border-radius: 12px; padding: 14px 18px; text-align: center; margin-bottom: 10px;
}
...
# ── Session state ──────────────────────────────────────────────────────────────
for key in ["pipeline_result", "chat_history", "step"]:
    ...
    for line in lines:
        if line:
            html += (f'<div class="item-row">'
                     f'<div class="item-dot {dot_class}"></div>'
                     f'<div class="item-text">{line}</div>'
                     f'</div>')
...
def run_pipeline_ui(source: str, language: str):
    ...
    def tick(idx):
        ...
        status.markdown(f"<p style='color:#6B7A99;font-size:0.85rem;font-family:Space Grotesk,sans-serif'>{msg}</p>",
                        unsafe_allow_html=True)
...
with st.sidebar:
    ...
    source = st.text_input(
        ...
        placeholder="https://youtube.com/watch?v=... or /path/to/file.mp3",
    )
...
with tab_transcript:
    ...
    st.download_button("⬇  Download transcript", data=result["transcript"],
...
with tab_chat:
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            st.markdown(f'<div class="chat-label">MeetMind</div>'
                        f'<div class="chat-bubble-ai">{msg["content"]}</div>',
                        unsafe_allow_html=True)

    col_q, col_btn = st.columns([5, 1])
    with col_q:
        question = st.text_input("q", placeholder="Ask anything about the meeting…",
    ...
    if not st.session_state.chat_history:
        st.markdown('<p style="color:#3B4562;font-size:0.82rem;margin-top:12px">'
